...
class OneFileLoginApplication
{
	...
	echo 'you are logged in.<br/><br/>';
        echo $_SESSION['user_name'] . 'さん<br/>';

	$unit_cnt = count($_SESSION['building_info']);
	echo 'You have ' . $unit_cnt . ' unit(s).<br/><br/>';
	for ($i = 0; $i < $unit_cnt; $i++){
	    print_r(
	    '名前 : ' . $_SESSION['building_info'][$i][building_name] .
	    ', 現在(peso) : '. $_SESSION['building_info'][$i][building_price] .
	    ', 当初(peso) : '. $_SESSION['building_info'][$i][building_initial_price]
	    );	    
	    echo '</br>';							    	    
	}	
	$parking_cnt = count($_SESSION['parking_info']);
	echo 'You have ' . $parking_cnt . ' parking(s).<br/><br/>';
	for ($i = 0; $i < $parking_cnt; $i++){
	    print_r(
	    '名前 : ' . $_SESSION['parking_info'][$i][parking_name] .
	    ', 現在(peso) : '. $_SESSION['parking_info'][$i][parking_price] .
	    ', 当初(peso) : '. $_SESSION['parking_info'][$i][parking_initial_price]
	    );	    
	    echo '</br>';							    	    
	}	
